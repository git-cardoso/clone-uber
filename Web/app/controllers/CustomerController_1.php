    ...
    public function create() {
        if (Request::isMethod('post')) {
            ...
            $validator = Validator::make(
                        ...
                        'token' => '',
                        'owner_id' => 'Your unique id is missing.',
                        /* 'picture' => 'required|mimes:jpeg,bmp,png', */
            ...
            if ($validator->fails()) {
                ...
                if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                    ...
                    if (is_token_active($owner_data->token_expiry) || $is_admin) {
                        ...
                        $response_array = array('success' => false, 'error' => 'Token Expired', 'error_code' => 405, 'error_messages' => array('Token Expired'));
                        $response_code = 200;
                    }
                } else {
                    if ($is_admin) {
                        $response_array = array('success' => false, 'error' => 'Owner ID not Found', 'error_messages' => array('Owner ID not Found'), 'error_code' => 410);
                    } else {
                        $response_array = array('success' => false, 'error' => 'Not a valid token', 'error_messages' => array('Not a valid token'), 'error_code' => 406);
                    }
                    $response_code = 200;
                }
            }
                ...
                if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                    ...
                    if (is_token_active($owner_data->token_expiry) || $is_admin) {
                        ...
                        if ($dog) {
                            ...
                            $response_array = array('success' => false, 'error' => 'No Dogs Found', 'error_messages' => array('No Dogs Found'), 'error_code' => 445);
                            $response_code = 200;
                        }
                    } else {
                        $response_array = array('success' => false, 'error' => 'Token Expired', 'error_messages' => array('Token Expired'), 'error_code' => 405);
                        $response_code = 200;
                    }
                } else {
                ...
                if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                    ...
                    if (is_token_active($owner_data->token_expiry) || $is_admin) {
                        ...
                        if ($dog_data) {
                            ...
                            $response_array = array('success' => false, 'error' => 'No Dog Found', 'error_messages' => array('No Dog Found'), 'error_code' => 405);
                            $response_code = 200;
                        }
                    } else {
                        $response_array = array('success' => false, 'error' => 'Token Expired', 'error_messages' => array('Token Expired'), 'error_code' => 405);
    ...
    public function set_walker_rating() {
        if (Request::isMethod('post')) {
            ...
            $validator = Validator::make(
                        ...
                        'request_id' => 'Id of Request is required.',
                        /* 'rating' => 'required|integer', */
                        'token' => '',
                        'owner_id' => 'Your unique id is missing.'
                            )
                            ...
                            if ($request->owner_id == $owner_data->id) {
                                if ($request->is_completed == 1) {
                                    if ($request->is_walker_rated == 0) {
                                        ...
                                        $response_array = array('success' => false, 'error' => 'Already Rated', 'error_messages' => array('Already Rated'), 'error_code' => 409);
                                        $response_code = 200;
                                    }
                                } else {
                                    $response_array = array('success' => false, 'error' => 'Walk is not completed', 'error_messages' => array('Walk is not completed'), 'error_code' => 409);
                                    $response_code = 200;
                                }
                            } else {
                                $response_array = array('success' => false, 'error' => 'Walk ID doesnot matches with Dog ID', 'error_messages' => array('Walk ID doesnot matches with Dog ID'), 'error_code' => 407);
                                $response_code = 200;
                            }
                        } else {
                            $response_array = array('success' => false, 'error' => 'Walk ID Not Found', 'error_messages' => array('Walk ID Not Found'), 'error_code' => 408);
                            $response_code = 200;
                        }
                    } else {
                        $response_array = array('success' => false, 'error' => 'Token Expired', 'error_messages' => array('Token Expired'), 'error_code' => 405);
    ...
    public function set_location() {
        if (Request::isMethod('post')) {
            ...
            $validator = Validator::make(
                        ...
                        'latitude' => 'location points are missing.',
                        'longitude' => 'location points are missing.',
                        'token' => '',
                        'owner_id' => 'Your unique id is missing.'
                            )
            );
            ...
            if ($validator->fails()) {
                ...
                if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                    ...
                    if ($is_admin) {
                        ...
                        $response_array = array('success' => false, 'error' => '' . Config::get('app.generic_keywords.User') . 'ID not Found', 'error_messages' => array('' . Config::get('app.generic_keywords.User') . 'ID not Found'), 'error_code' => 410);
                    } else {
                        $response_array = array('success' => false, 'error' => 'Not a valid token', 'error_messages' => array('Not a valid token'), 'error_code' => 406);
                    }
                    $response_code = 200;
...
// Get Walk Location
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    if ($request = Requests::find($request_id)) {
                        if ($request->owner_id == $owner_id) {
                            ...
                            $response_array = array('success' => false, 'error' => 'Request ID doesnot matches with' . Config::get('app.generic_keywords.User') . ' ID', 'error_messages' => array('Request ID doesnot matches with' . Config::get('app.generic_keywords.User') . ' ID'), 'error_code' => 407);
                            $response_code = 200;
                        }
                    } else {
                        $response_array = array('success' => false, 'error' => 'Request ID Not Found', 'error_messages' => array('Request ID Not Found'), 'error_code' => 408);
                        $response_code = 200;
                    }
                } else {
                    $response_array = array('success' => false, 'error' => 'Token Expired', 'error_messages' => array('Token Expired'), 'error_code' => 405);
    ...
    public function get_providers_all() {
        ...
        if ($validator->fails()) {
            ...
            if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    if ($walkers != NULL) {
                        ...
                        $response_array = array(
                            ...
                            'error_messages' => array('No walker found'),
                        );
                        $response_code = 200;
                    }
                } else {
                ...
                if ($is_admin) {
                    /* $response_array = array('success' => false, 'error' => '' . $var->keyword . ' ID not Found', 'error_code' => 410); */
                    $response_array = array('success' => false, 'error_messages' => array('' . Config::get('app.generic_keywords.User') . ' ID not Found'), 'error' => '' . Config::get('app.generic_keywords.User') . ' ID not Found', 'error_code' => 410);
                } else {
                    $response_array = array('success' => false, 'error_messages' => array('Not a valid token'), 'error' => 'Not a valid token', 'error_code' => 406);
                }
                $response_code = 200;
            }
        }
            ...
            if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    if ($typewalkers == NULL) {
                        ...
                        $response_array = array('success' => false, 'error' => 'No ' . Config::get('app.generic_keywords.Provider') . ' found matching the service type.', 'error_messages' => array('No ' . Config::get('app.generic_keywords.Provider') . ' found matching the service type.'), 'error_code' => 405);
                        $response_code = 200;
                    ...
                    if ($walkers != NULL) {
                        ...
                        $response_array = array(
                            'success' => false,
                            'unit' => $unit_set,
                            'error' => 'No ' . Config::get('app.generic_keywords.Provider') . ' found',
                            'error_messages' => array('No ' . Config::get('app.generic_keywords.Provider') . ' found'),
                        );
                        $response_code = 200;
                    }
                } else {
    ...
    public function create_request_providers() {
        ...
        $validator = Validator::make(
                    ...
                    'token' => '',
                    'owner_id' => 'Your unique id is missing.',
                    'provider_id' => 'Selected provider\'s unique id is missing.',
                        )
        );
        /* $var = Keywords::where('id', 2)->first(); */
        ...
        if ($validator->fails()) {
            ...
            if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    $response_array = array('success' => false, 'error' => 'Token Expired', 'error_messages' => array('Not a valid token'), 'error_code' => 405);
                    $response_code = 200;
                }
            } else {
                if ($is_admin) {
            ...
            if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    $request = DB::table('request')->where('owner_id', $owner_data->id)
                            ...
                            ->first();
                    if ($request) {
                        $response_array = array('success' => false, 'error' => 'Your previous Request is Pending.', 'error_messages' => array('Your previous Request is Pending.'), 'error_code' => 405);
                        $response_code = 200;
                    } else {
                        /* SEND REFERRAL & PROMO INFO */
                        if ($payment_opt != 1) {
                            ...
                            if ($card_count <= 0) {
                                $response_array = array('success' => false, 'error' => "Please add card first for payment.", 'error_messages' => array("Please add card first for payment."), 'error_code' => 417);
                                $response_code = 200;
                                $response = Response::json($response_array, $response_code);
                                return $response;
                            }
                        ...
                        /* if ($owner_data->debt > 0) {
                          $response_array = array('success' => false, 'error' => "You are already in \$$owner_data->debt debt", 'error_messages' => array("You are already in \$$owner_data->debt debt"), 'error_code' => 417);
                          $response_code = 200;
                          $response = Response::json($response_array, $response_code);
                          return $response;
                          } */
                                        ...
                                        if ($prom_act_card) {
                                            if ($promos = PromoCodes::where('coupon_code', $promo_code)->where('uses', '>', 0)->where('state', '=', 1)->first()) {
                                                if ((date("Y-m-d H:i:s") >= date("Y-m-d H:i:s", strtotime(trim($promos->expiry)))) || (date("Y-m-d H:i:s") <= date("Y-m-d H:i:s", strtotime(trim($promos->start_date))))) {
                                                    $response_array = array('success' => FALSE, 'error' => 'Promotional code is not available', 'error_messages' => array('Promotional code is not available'), 'error_code' => 505);
                                                    $response_code = 200;
                                                    return Response::json($response_array, $response_code);
                                                } else {
                                            ...
                                            $response_array = array('success' => FALSE, 'error' => 'Promotion feature is not active on card payment.', 'error_messages' => array('Promotion feature is not active on card payment.'), 'error_code' => 505);
                                            $response_code = 200;
                                            return Response::json($response_array, $response_code);
                                        }
                                        ...
                                        if ($prom_act_cash) {
                                            if ($promos = PromoCodes::where('coupon_code', $promo_code)->where('uses', '>', 0)->where('state', '=', 1)->first()) {
                                                if ((date("Y-m-d H:i:s") >= date("Y-m-d H:i:s", strtotime(trim($promos->expiry)))) || (date("Y-m-d H:i:s") <= date("Y-m-d H:i:s", strtotime(trim($promos->start_date))))) {
                                                    ...
                                                    $promo_is_used = UserPromoUse::where('user_id', '=', $owner_id)->where('code_id', '=', $promos->id)->count();
                                                    if ($promo_is_used) {
                                                        $response_array = array('success' => FALSE, 'error' => 'Promotional code already used.', 'error_messages' => array('Promotional code is not available'), 'error_code' => 512);
                                                        $response_code = 200;
                                                        return Response::json($response_array, $response_code);
                                                    } else {
                                                        $promo_update_counter = PromoCodes::find($promos->id);
                                            ...
                                            $response_array = array('success' => FALSE, 'error' => 'Promotion feature is not active on cash payment.', 'error_messages' => array('Promotion feature is not active on cash payment.'), 'error_code' => 505);
                                            $response_code = 200;
                                            return Response::json($response_array, $response_code);
                                        }
                                    }/* else {
                                      $response_array = array('success' => FALSE, 'error' => 'Payment mode is paypal','error_messages' => array('Payment mode is paypal'), 'error_code' => 505);
                                      $response_code = 200;
                                      return Response::json($response_array, $response_code);
                                      } */
                                } else {
                                    $response_array = array('success' => FALSE, 'error' => 'Promotion feature is not active.', 'error_messages' => array('Promotion feature is not active.'), 'error_code' => 505);
                                    $response_code = 200;
                                    return Response::json($response_array, $response_code);
                                }
                        ...
                        if ($walker) {
                            ...
                            $response_array = array('success' => false, 'error' => 'No ' . Config::get('app.generic_keywords.Provider') . ' found for the selected service in your area currently', 'error_messages' => array('No ' . Config::get('app.generic_keywords.Provider') . ' found for the selected service in your area currently'), 'error_code' => 415);
                            $response_code = 200;
                            return Response::json($response_array, $response_code);
                        }
                        // Send SMS 
        ...
        if ($validator->fails()) {
            ...
            if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    if ($owner_data->debt > 0) {
                        $response_array = array('success' => false, 'error' => "You are already in \$$owner->debt debt", 'error_messages' => array("You are already in \$$owner->debt debt"), 'error_code' => 417);
                        $response_code = 200;
                        $response = Response::json($response_array, $response_code);
                        return $response;
                    }
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    if (!empty($request_data)) {
                        $response_array = array(
                            ...
                            'error_messages' => array('On going ' . Config::get('app.generic_keywords.Trip') . '.'),
                            'error' => 'On going ' . Config::get('app.generic_keywords.Trip') . '.',
                        );
                    } else {
                        $response_array = array(
                            'success' => false,
                            ...
                            'error_code' => 584,
                            /* 'error' => 'Searching for ' . $driver->keyword . 's.', */
                            'error_messages' => array('No on going ' . Config::get('app.generic_keywords.Trip') . ' found.'),
                            'error' => 'No on going ' . Config::get('app.generic_keywords.Trip') . ' found.',
                        );
                    }
                    $response_code = 200;
                } else {
                            ...
                            if ($request->current_walker != 0) {
                                ...
                                if ($request->confirmed_walker != 0) {
                                    ...
                                    $response_array = array(
                                        ...
                                        'error_messages' => array('Searching for ' . Config::get('app.generic_keywords.Provider') . 's.'),
                                    );
                                }
                            } else {
                                /* $driver = Keywords::where('id', 1)->first(); */
                                ...
                                $response_array = array(
                                    ...
                                    'error_messages' => array('No ' . Config::get('app.generic_keywords.Provider') . 's are available currently. Please try after sometime.'),
                                );
                            }
                            $response_code = 200;
                        } else {
                    ...
                    if ($request = Requests::find($request_id)) {
                        ...
                        if ($request->owner_id == $owner_data->id) {
                            ...
                            if ($request->confirmed_walker != 0) {
                                ...
                                $response_array = array(
                                    'success' => false,
                                    'error' => '' . Config::get('app.generic_keywords.Provider') . ' not Confirmed yet',
                                    'error_messages' => array('' . Config::get('app.generic_keywords.Provider') . ' not Confirmed yet'),
                                    'error_code' => 421,
                                );
                            }
                            $response_code = 200;
    ...
    public function schedule_future_request() {
        ...
        foreach ($auto_cancled_results as $remove_schedules) {
            ...
            /* SMS NOTIFICATION */
            sms_notification($owner_data->owner_id, 'owner', 'Hello...!, ' . $owner_data->first_name . ' your scheduled ' . Config::get('app.generic_keywords.Trip') . ' was auto canceled because no ' . $driver_keyword . ' are available right now in your area.');
                    ...
                    if ($transfer_amount > 0) {
                        ...
                        try {
                            ...
                            if ($transfer->status != 'canceled' || $transfer->status != 'failed') {
                                ...
                                $walker_data->payment_remaining = $walker_data->payment_remaining - $transfer_floor - floor($provider_data_trans->refund_remaining);
                                $walker_data->refund_remaining = $walker_data->refund_remaining - floor($walker_data->refund_remaining);
                                $walker_data->save();
    ...
    public function create_future_request() {
        ...
        $validator = Validator::make(
                    ...
                    'start_time' => 'Schedule date and time must be required.',
                        )
        );
        if ($validator->fails()) {
            $error_messages = $validator->messages()->all();
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    if ($chk_dt->invert == 0 && $chk_dt->d >= 0 && $chk_dt->d <= 14) {
                        ...
                        /* $response_array = array(
                          ...
                          'error' => "Please add card first for payment.",
                          'error_messages' => array("Please add card first for payment."),
                          'error_code' => 417
                          );
                          $response_code = 200;
                          $response = Response::json($response_array, $response_code);
                            ...
                            if ($prom_act) {
                                if ($payment_mode == 0) {
                                    ...
                                    if ($prom_act_card) {
                                        if ($promos = PromoCodes::where('coupon_code', $promo_code)->where('uses', '>', 0)->where('state', '=', 1)->first()) {
                                            if ((date("Y-m-d H:i:s") >= date("Y-m-d H:i:s", strtotime(trim($promos->expiry)))) || (date("Y-m-d H:i:s") <= date("Y-m-d H:i:s", strtotime(trim($promos->start_date))))) {
                                                ...
                                                $response_array = array(
                                                    'success' => FALSE,
                                                    'error' => 'Promotional code is not available',
                                                    'error_messages' => array('Promotional code is not available'),
                                                    'error_code' => 505,
                                                    /* 'now_dt' => $chk_dt, */
                                                    'is_referral_active' => $referral_code_activation,
                                                    'is_referral_active_txt' => $referral_code_activation_txt,
                                        ...
                                        $response_array = array(
                                            'success' => FALSE,
                                            'error' => 'Promotion feature is not active on card payment.',
                                            'error_messages' => array('Promotion feature is not active on card payment.'),
                                            'error_code' => 505,
                                            /* 'now_dt' => $chk_dt, */
                                            'is_referral_active' => $referral_code_activation,
                                            'is_referral_active_txt' => $referral_code_activation_txt,
                                    ...
                                    if ($prom_act_cash) {
                                        ...
                                        $response_array = array(
                                            'success' => FALSE,
                                            'error' => 'Promotion feature is not active on cash payment.',
                                            'error_messages' => array('Promotion feature is not active on cash payment.'),
                                            'error_code' => 505,
                                            /* 'now_dt' => $chk_dt, */
                                            'is_referral_active' => $referral_code_activation,
                                            'is_referral_active_txt' => $referral_code_activation_txt,
                                ...
                                $response_array = array(
                                    'success' => FALSE,
                                    'error' => 'Promotion feature is not active.',
                                    'error_messages' => array('Promotion feature is not active.'),
                                    'error_code' => 505,
                                    /* 'now_dt' => $chk_dt, */
                                    'is_referral_active' => $referral_code_activation,
                                    'is_referral_active_txt' => $referral_code_activation_txt,
                        ...
                        $response_array = array(
                            'success' => false,
                            'error' => "Sorry, You can't create schedule morethen two week faar from today.",
                            'error_messages' => array("Sorry, You can't create schedule morethen two week faar from today."),
                            'error_code' => 506,
                            /* 'now_dt' => $chk_dt, */
                            'is_referral_active' => $referral_code_activation,
                            'is_referral_active_txt' => $referral_code_activation_txt,
                    ...
                    $response_array = array(
                        'success' => false,
                        'error' => 'Token Expired',
                        'error_messages' => array('Token Expired'),
                        'error_code' => 405,
                        'is_referral_active' => $referral_code_activation,
                        'is_referral_active_txt' => $referral_code_activation_txt,
                        'is_promo_active' => $promotional_code_activation,
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    if ($request = ScheduledRequests::find($request_id)) {
                        if ($request->owner_id == $owner_data->id) {
                            ...
                            $response_array = array(
                                'success' => false,
                                'error' => 'Request ID doesnot matches with ' . Config::get('app.generic_keywords.User') . ' ID',
                                'error_messages' => array('Request ID doesnot matches with ' . Config::get('app.generic_keywords.User') . ' ID'),
                                'error_code' => 407,
                                'is_referral_active' => $referral_code_activation,
                                'is_referral_active_txt' => $referral_code_activation_txt,
                                'is_promo_active' => $promotional_code_activation,
                        ...
                        $response_array = array(
                            ...
                            'error_messages' => array('Request ID Not Found'),
                            'error_code' => 408,
                            'is_referral_active' => $referral_code_activation,
                            'is_referral_active_txt' => $referral_code_activation_txt,
                            'is_promo_active' => $promotional_code_activation,
    ...
    public function create_request_later() {
        ...
        $validator = Validator::make(
                    ...
                    'latitude' => 'location points are missing.',
                    'longitude' => 'location points are missing.',
                    'datetime' => 'Schedule date and time must be required.',
                        )
        );
    ...
    public function eta() {
        ...
        $validator = Validator::make(
                    ...
                    'secret' => 'Security key is required.',
                    'token' => '',
                    'owner_id' => 'Your unique id is missing.',
                        )
        );
    ...
    public function check_promo_code() {
        ...
        $validator = Validator::make(
                    ...
                    'promo_code' => 'Promotional code must be required.',
                    'token' => '',
                    'owner_id' => 'Your unique id is missing.',
                        )
        );
                    ...
                    if ($prom_act) {
                        ...
                        if ($check_code != NULL) {
                            if ($check_code->state == 1 && date('Y-m-d H:i:s', strtotime($check_code->expiry)) > date('Y-m-d H:i:s') && date('Y-m-d H:i:s', strtotime($check_code->start_date)) <= date('Y-m-d H:i:s')) {
                                ...
                                $response_array = array('success' => false, 'error' => 'Invalid Promo Code', 'error_messages' => array('Invalid Promo Code'), 'error_code' => 418);
                            }
                        } else {
                            $response_array = array('success' => false, 'error' => 'Invalid Promo Code', 'error_messages' => array('Invalid Promo Code'), 'error_code' => 419);
                        }
                    } else {
                        $response_array = array('success' => false, 'error' => 'Promotion feature is not active.', 'error_messages' => array('Promotion feature is not active.'), 'error_code' => 419);
                    }
                } else {
                    $response_array = array('success' => false, 'error' => 'Request ID Not Found', 'error_messages' => array('Request ID Not Found'), 'error_code' => 408);
                }
    ...
    public function payment_select() {
        ...
        $validator = Validator::make(
                    ...
                    'payment_select' => 'Payment type must be required.',
                    'owner_id' => 'Your unique id is missing.'
                        )
        );
        //echo "test";
        ...
        if ($validator->fails()) {
            ...
            if ($request) {
                if (isset($request->id)) {
                    ...
                    $response_array = array('success' => false, 'error' => 'Payment mode not updated', 'error_messages' => array('Payment mode not updated'), 'error_code' => 507);
                    $response_code = 200;
                }
            } else {
                $response_array = array('success' => false, 'error' => 'Payment mode not updated', 'error_messages' => array('Payment mode not updated'), 'error_code' => 507);
    ...
    public function get_provider_list() {
        ...
        $validator = Validator::make(
                    ...
                    'usr_lat' => 'location points are missing.',
                    'user_long' => 'location points are missing.',
                        )
        );
    ...
    public function user_set_destination() {
        ...
        $validator = Validator::make(
                    ...
                    'token' => '',
                    'owner_id' => 'Your unique id is missing.',
                    'dest_lat' => 'location points are missing.',
                    'dest_long' => 'location points are missing.',
                        )
        );
        if ($validator->fails()) {
            $error_messages = $validator->messages()->all();
                        ...
                        if ($request->owner_id == $owner_data->id) {
                            if ($request->promo_id == 0) {
                                ...
                                $response_array = array(
                                    ...
                                    'error_messages' => array("Destination Set Successfully"),
                                );
                                $response_code = 200;
                            } else {
    ...
    public function user_create_event() {
        ...
        $validator = Validator::make(
                    ...
                    'latitude' => 'location points are missing.',
                    'longitude' => 'location points are missing.',
                    'name' => 'Event name is missing.',
                    'event_date_time' => 'Event Date & time are missing.',
                    'member_number' => 'Numbe of Event Members is missing.',
                    'event_time_zone' => 'Event country timezone is missing.',
                        )
        );
        if ($validator->fails()) {
            $error_messages = $validator->messages()->all();
            ...
            if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    if ($chk_dt->invert != 0 && ($chk_dt->d >= 14 || $chk_dt->m > 0)) {
                        ...
                        if (Config::get('app.default_payment') == 'stripe') {
                            ...
                            try {
                                $charge = Stripe_Charge::create(array(
                                            "amount" => floor($pre_pay_for_each_member * $member_number) * 100,
                                            "currency" => "usd",
                                ...
                                if ($charge->paid) {
                                    ...
                                    $response_array = array(
                                        'success' => false,
                                        'error' => 'Card Payment Fail, Please try after sometime.',
                                        'error_messages' => array('Card Payment Fail, Please try after sometime.'),
                                        'error_code' => 405,
                                        'is_referral_active' => $referral_code_activation,
                                        'is_referral_active_txt' => $referral_code_activation_txt,
                                        'is_promo_active' => $promotional_code_activation,
                        ...
                        $response_array = array(
                            'success' => false,
                            'error' => 'Sorry You can\'t create event earlier then 14 days.',
                            'error_messages' => array('Sorry You can\'t create event earlier then 14 days.'),
                            'error_code' => 405,
                            'is_referral_active' => $referral_code_activation,
                            'is_referral_active_txt' => $referral_code_activation_txt,
                            'is_promo_active' => $promotional_code_activation,
    ...
    public function invite_members() {
        ...
        $validator = Validator::make(
                    ...
                    'token' => '',
                    'owner_id' => 'Your unique id is missing.',
                    'event_id' => 'Event id was missing.',
                    'email' => 'Invited user\'s email id was missing.',
                        )
        );
        if ($validator->fails()) {
            $error_messages = $validator->messages()->all();
            ...
            $response_array = array('success' => TRUE, 'emails' => $email);
            $response_code = 200;
        }
        $response = Response::json($response_array, $response_code);
        return $response;
