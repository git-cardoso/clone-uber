        ...
        if ($validator->fails()) {
            ...
            if ($walker_data = $this->getWalkerData($walker_id, $token, $is_admin)) {
                ...
                if (is_token_active($walker_data->token_expiry)) {
                    ...
                    $query = "SELECT request.*,id, later, D_latitude, D_longitude, payment_mode, request_start_time , owner_id,TIMESTAMPDIFF(SECOND,updated_at, '$time') as diff from request where is_cancelled = 0 and status = 0 and current_walker=$walker_id and TIMESTAMPDIFF(SECOND,updated_at, '$time') <= $timeout";
